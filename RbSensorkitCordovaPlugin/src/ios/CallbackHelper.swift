...
class CallbackHelper {
  ...
  func sendNumber(_ command: CDVInvokedUrlCommand, _ number: Int, _ keepCallback: Bool = false) {
    ...
    // Returns an OK response with a number value
    func sendBool(_ command: CDVInvokedUrlCommand, _ value: Bool, _ keepCallback: Bool = false) {
      let pluginResult = CDVPluginResult(
        status: CDVCommandStatus_OK,
        messageAs: value
      )
  ...
  func sendJson(_ command: CDVInvokedUrlCommand, _ json: [String:AnyObject]?, _ keepCallback: Bool = false) {
    ...
    // Returns an Error response with a JSON object
    func sendErrorJson(_ command: CDVInvokedUrlCommand, _ json: [String:AnyObject]?, _ keepCallback: Bool = false) {
      let pluginResult = CDVPluginResult(
        status: CDVCommandStatus_ERROR,
        messageAs: json
      )
